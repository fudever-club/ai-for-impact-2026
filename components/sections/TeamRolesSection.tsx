...
export const TeamRolesSection: React.FC<TeamRolesSectionProps> = ({
  ...
  return (
    <section id="team-roles" className="py-24 relative bg-slate-50/60 border-t border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Warning Banner */}
        <div className="mb-12 max-w-4xl mx-auto glass-card p-5 sm:p-6 rounded-2xl border border-amber-200/90 shadow-card bg-amber-50/50 flex items-start gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {team.roles.map((role, idx) => (
            <div
              key={idx}
              className="glass-card p-6 sm:p-7 rounded-2xl border border-slate-200/90 shadow-card hover:shadow-card-hover hover:border-brand-orange/40 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group bg-white"
            >
              <div>
                <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
                  {roleIcons[idx] || <Users className="w-6 h-6" />}
                </div>
        ...
        {/* 5th Member Note */}
        <div className="mb-10 text-center max-w-2xl mx-auto p-3.5 rounded-xl bg-blue-50/70 border border-blue-100 text-xs sm:text-sm text-blue-800 font-medium">
          💡 {team.fifthMemberNote}
        </div>

        {/* Mock Data Security Policy Card */}
        <div className="glass-card p-6 sm:p-7 rounded-2xl border border-blue-200/90 shadow-card max-w-4xl mx-auto bg-gradient-to-r from-blue-50/70 via-white to-sky-50/50 flex items-start gap-4">
          <div className="w-12 h-12 rounded-xl bg-blue-100/80 border border-blue-200 flex items-center justify-center text-blue-600 shrink-0 mt-0.5 shadow-xs">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="blue" className="text-[10px]">Quy tắc An toàn</Badge>
            </div>
