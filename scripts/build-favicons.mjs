...
// Official FPT 3-color symbol vector
const svgContent = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="64" height="64">
  <!-- Rounded clean white backdrop tile for dark/light browser tab contrast -->
  <rect width="64" height="64" rx="14" fill="#FFFFFF"/>
  <g transform="translate(6, 14.5) scale(1.53)">
    <!-- Blue petal - F -->
...
// Build standard multi-resolution ICO file from PNG buffers
...
fs.writeFileSync('public/apple-touch-icon.png', png256);
fs.writeFileSync('public/icon.png', png64);
fs.writeFileSync('app/icon.png', png64);
fs.writeFileSync('app/icon.svg', svgContent);
fs.writeFileSync('public/icon.svg', svgContent);
fs.writeFileSync('public/icons/fpt-symbol.svg', svgContent);

console.log('Successfully generated official FPT 3-color favicons:');
console.log('- app/favicon.ico (' + icoBuffer.length + ' bytes)');
console.log('- public/favicon.ico (' + icoBuffer.length + ' bytes)');
console.log('- public/apple-touch-icon.png (' + png256.length + ' bytes)');
console.log('- app/icon.svg and public/icon.svg');
